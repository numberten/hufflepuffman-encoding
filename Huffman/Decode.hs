...
decode bytes    = readEncoding encoded_bits
  where
    unpacked_bytes  = BL.unpack bytes
    encoded_bits    = removePadding
                    . concat
                    $ map word8ToBits unpacked_bytes

-- | Decodes a ByteString that has been encoded with
-- both a header and padding.
...
-- | Read a list of bits and convert them into values
-- using a given BTree.
...
-- | Turn a Word8 into eight Word8s that represent the zeros
-- and ones used to make it.
word8ToBits :: Word8 -> [Word8]
word8ToBits = helper 7
  where
